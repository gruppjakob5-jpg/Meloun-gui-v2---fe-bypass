...
-- Instances:
local Converted = {
	["_MelounXecutor"] = Instance.new("ScreenGui");
...
local function ZXOSLR_fake_script() -- Fake Script: StarterGui.Starlight.Frame.LocalScript
	...
	local function fastFindRemote(timeout)
		...
		for i = 1, #remotes do
			...
			task.spawn(function()
				...
				if ok and result then
					print("MelounXecotur: backdoor found:", remotes[i]:GetFullName())
				end
		...
		if not foundExploit then
			print("MelounXecutort: backdoor not found")
		end
	...
	local function findRemote()
		...
		FinishedFound = true
		print(string.format("MelounXecutor: scan completed in %.3f seconds", os.clock() - tStart))
	end
	...
	showNotification("starlight 💫", "We're back! ", 5)
	...
	script.Parent.Framee.scan.MouseButton1Click:Connect(function()
		...
		scanning = true
		FinishedFound = false
		showNotification("MelounXecutor", "scanning...", 99)
